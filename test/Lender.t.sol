...
import {Pool, Loan, Staked, Borrow} from "../src/utils/Structs.sol";
...
contract LenderTest is Test {
    ...
    function test_shouldBeAbleToDeployPoolWithCollateralAddressZero() public {
        vm.startPrank(lender1);
        Pool memory p = Pool({
            lender: lender1,
            loanToken: address(loanToken),
        ...
        bytes32 returnedPoolId = lender.setPool(p);
        vm.stopPrank();
    ...
    function test_check1() public {
        ...
        vm.prank(user);
        loanToken.approve(address(lender), 120 * 10 ** 18);
        vm.startPrank(user);
        uint256[] memory repayId = new uint256[](1);
        repayId[0] = 0;
